...
segment `ds` at `(labels, rip, s)` demands `Effects.All` of the postcondition
...
def Directives.wpTrans (ds : List (Directive × Nat)) :
    PredTrans (Labels → Int64 → MachineData → Prop) EPost.Nil Unit :=
  ⟨fun Q _E labels rip s =>
    (@Directives.interp labels ds s rip (fun pc s' => .done (s', pc))).All
      (fun st => Q () labels st.2 st.1)⟩

instance instWPDirectives :
    WP (List (Directive × Nat)) Unit (Labels → Int64 → MachineData → Prop) EPost.Nil where
  wpTrans := Directives.wpTrans
  wp_trans_monotone _ _ _ _ _ _ hQ := fun labels _ _ =>
    Effects.All.mono (fun st => hQ () labels st.2 st.1) _

@[simp] theorem Directives.wp_nil (Q : Unit → Labels → Int64 → MachineData → Prop)
    (E : EPost.Nil) (labels : Labels) (rip : Int64) (s : MachineData) :
    wp ([] : List (Directive × Nat)) Q E labels rip s = Q () labels rip s := rfl
...
theorem straightlineStep_of_wp [Layout] {e : Executable} {s : MachineData} {pc : Int64}
    ...
    (h : wp (e.directivesFromAddress pc) (fun _ _ pc' s' => post (s', pc'))
      EPost.Nil.mk e.labels pc s) :
...
theorem Directives.wp_eq (ds : List (Directive × Nat))
    (Q : Unit → Labels → Int64 → MachineData → Prop) (E : EPost.Nil)
    (labels : Labels) (rip : Int64) (s : MachineData) :
    wp ds Q E labels rip s =
      (@Directives.interp labels ds s rip (fun pc s' => .done (s', pc))).All
        (fun st => Q () labels st.2 st.1) := rfl

section Specs

variable {Q : Unit → Labels → Int64 → MachineData → Prop} {E : EPost.Nil}
...
@[spec] theorem Directives.nil_spec :
    ⦃ fun labels rip s => Q () labels rip s ⦄ (([] : List (Directive × Nat))) ⦃ Q; E ⦄ :=
  Triple.intro fun _ _ _ h => h

@[spec] theorem Directives.label_spec (l : Label) (sz : Nat) :
    ⦃ fun labels rip s => wp ds Q E labels (rip + .ofNat sz) s ⦄
