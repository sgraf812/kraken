...
theorem dynamic_stack_correct [CodeEnv] (stack : List UInt8) (lstack : stack.length = 1024)
  ...
  have r42 : BitVec.ofInt 64 (Int.ofBytes (Int.toBytes 8 42)) = 42#64 := by decide
  have r99 : BitVec.ofInt 64 (Int.ofBytes (Int.toBytes 8 99)) = 99#64 := by decide
  vcgen [dynamic_stack]
  all_goals first
    | exact SepWP.frames_directive _ _
    | (simp only [Int.toBytes_length]; done)
    | (subst_vars; unfold MProp.SliceBound
       simp only [off_saved, off_slot, List.length_take, lstack]
       bv_omega)
    | (subst_vars
       simp only [off_saved, off_slot, List.length_take, List.length_drop, lstack]
       bv_omega)
    | (refine Lean.Order.le_ofProp _ _ ?_; subst_vars; simp [r42, r99])
