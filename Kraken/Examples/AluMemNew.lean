...
theorem alu_mem_correct (env₀ : Env) (s₀ : MachineData) (v : Int)
    ...
    (h_mapped : Mem.loadInt s₀.dmem
        (AddrExpr.interp env₀.labels (.mk .W64) aeRdx136 s₀.regs (.mk 0 0)) 8 = some v)
    (h_back : Mem.loadInt
        (Mem.storeInt s₀.dmem (AddrExpr.interp env₀.labels (.mk .W64) aeRdx136 s₀.regs (.mk 0 0)) 8 42)
        (AddrExpr.interp env₀.labels (.mk .W64) aeRdx136 s₀.regs (.mk 0 0)) 8 = some 42) :
